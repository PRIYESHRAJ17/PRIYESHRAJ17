# Hi, I'm Priyesh Raj 👋

🎓 1st Year B.E. CSE (Data Science) @ DSATM, Bengaluru  
🎯 Targeting AI/ML & Data Science roles  
🤖 Currently building: ML projects & DSA foundations  
♟️ Chess player with international tournament experience

---

## 🛠️ Tech Stack

![C](https://img.shields.io/badge/C-00599C?style=flat&logo=c&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Arduino](https://img.shields.io/badge/Arduino-00979D?style=flat&logo=arduino&logoColor=white)
![ESP32](https://img.shields.io/badge/ESP32-E7352C?style=flat&logo=espressif&logoColor=white)

---

## 🚀 Projects

| Project | Description | Tech |
|---|---|---|
| [Mini Reconnaissance Robot](https://github.com/PRIYESHRAJ17/Mini-Reconnaissance-Robot) | Wi-Fi controlled robot with live camera stream, sensor telemetry & web dashboard | Arduino, ESP32-CAM, C++ |

*More coming soon — ML & Data Science projects in progress*

---

## 📈 Currently Learning

- 🐍 NumPy, Pandas, Matplotlib
- 🤖 Scikit-learn & ML fundamentals
- 💻 Data Structures & Algorithms (LeetCode)

---

## 📫 Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/priyeshraj17)

---

*"Building from scratch, one project at a time."*
